...
class TransactionsClient(BaseTransactionsClient):
    ...
    def create_item(self, model: stac_types.Item, **kwargs):
        ...
        with self.client.start_session(causal_consistency=True) as session:
            ...
            if self.client.stac.stac_item.count_documents(
                {"id": model["id"], "collection": model["collection"]}, limit=1, session=session
            ):
                raise ConflictError(
    ...
    def create_collection(self, model: stac_types.Collection, **kwargs):
        ...
        with self.client.start_session(causal_consistency=True) as session:
            if self.client.stac.stac_collection.count_documents({"id": model["id"]}, limit=1, session=session):
        ...
        with self.client.start_session(causal_consistency=True) as session:
            ...
            if (
                self.client.stac.stac_item.count_documents(
                    {"id": model["id"], "collection": model["collection"]}, session=session
                )
                == 0
            ):
                raise NotFoundError(
            ...
            self.delete_item(item_id=model["id"], collection_id=model["collection"], session=session)
        now = datetime.utcnow().strftime(DATETIME_RFC339)
    ...
    def update_collection(self, model: stac_types.Collection, **kwargs):
        """Update collection."""
        with self.client.start_session(causal_consistency=True) as session:
            if self.client.stac.stac_collection.count_documents({"id": model["id"]}, session=session) == 0:
    ...
    def delete_item(self, item_id: str, collection_id: str, **kwargs):
        ...
        with self.client.start_session(causal_consistency=True) as session:
            ...
            self.client.stac.stac_item.delete_one({"id": item_id, "collection": collection_id}, session=session)

    def delete_collection(self, collection_id: str, **kwargs):
        """Delete collection."""
        with self.client.start_session(causal_consistency=True) as session:
            if self.client.stac.stac_collection.count_documents({"id": collection_id}, session=session) == 0:
                raise NotFoundError(f"Collection {collection_id} does not exist")
            self.client.stac.stac_collection.delete_one({"id": collection_id}, session=session)
