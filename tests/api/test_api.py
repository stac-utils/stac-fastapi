...
from stac_api.errors import add_exception_handlers, DEFAULT_STATUS_CODES
...
def test_core_router(postgres_core):
    ...
    assert STAC_CORE_ROUTES[1:] == sorted([f"{list(route.methods)[0]} {route.path}" for route in router.routes])


def test_transactions_router(postgres_transactions):
    settings = ApiSettings()
    router = create_transactions_router(postgres_transactions, settings)
    assert STAC_TRANSACTION_ROUTES == sorted([f"{list(route.methods)[0]} {route.path}" for route in router.routes])


def test_app_transaction_extension(load_test_data):
    settings = ApiSettings(
        stac_api_extensions=["transaction"]
    )
...
def test_app_sort_extension(load_test_data, postgres_transactions):
    ...
    item_date = datetime.strptime(first_item["properties"]["datetime"], "%Y-%m-%dT%H:%M:%SZ")
    postgres_transactions.create_item(Item.parse_obj(first_item), request=MockStarletteRequest)

    second_item = load_test_data("test_item.json")
    second_item["id"] = "another-item"
    another_item_date = item_date - timedelta(days=1)
    second_item["properties"]["datetime"] = another_item_date.strftime("%Y-%m-%dT%H:%M:%SZ")
    postgres_transactions.create_item(Item.parse_obj(second_item), request=MockStarletteRequest)
